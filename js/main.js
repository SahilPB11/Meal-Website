...
function showMeals() {
  ...
  let meals = fetchMealsFromApi(url, searchBarValue).then((data) => {
    if (data.meals) {
      data.meals.map((element) => {
        let isFav = false;
        for (let i = 0; i < arr.length; i++) {
          if (arr[i] == element.idMeal) isFav = true;
        }
        if (isFav) {
          html += `
            <div id="card" class="card m-3" style="width: 20rem;">
                ...
                <div class="card-body card2-body">
                    ...
                    <div class="d-flex justify-content-between mt-5">
                        <button type="button" class="btn btn-outline-light card2-btn" onclick="showMealDetails(${element.idMeal})">More Details</button>
          ...
          html += `
            <div id="card" class="card card-hover m-3" style="width: 20rem;">
                <img src="${element.strMealThumb}" class="card-img-top " alt="...">
                <div class="card-body card2-body">
                    <h5 class="card-title">${element.strMeal}</h5>
                    <div class="d-flex justify-content-between mt-5"
                    style="width:100%;">
                        ...
                        <button id="main${element.idMeal}" class="btn btn-outline-light " onclick="addRemoveToFavList(${element.idMeal})" style="border-radius:50%"><i class="fa-solid fa-heart" style="color:  #ff0000;"></i></button>
                    </div>
                </div>
            </div>
            `;
...
async function showFavMealList() {
                ...
                <div id="card" class="card mb-3" style="width: 20rem;">
                    ...
                    <div class="card-body card2-body">
                        ...
                        <div class="d-flex justify-content-between mt-5">
                            ...
                            <button id="main${data.meals[0].idMeal}" class="btn btn-outline-light active" onclick="addRemoveToFavList(${data.meals[0].idMeal})" style="border-radius:50%"></button>
                        </div>
                    </div>
                </div>
                `;
  ...
  document.getElementById("favourite-body").innerHTML = html;
}
